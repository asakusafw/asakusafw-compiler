...
/**
 * A Gradle sub plug-in for Asakusa Language projects.
 * @since 0.4.2
 */
class AsakusaLangBasePlugin implements Plugin<Project> {

    private static final String ARTIFACT_INFO_PATH = 'META-INF/asakusa-lang-gradle/artifact.properties'

    private static final String INVALID_VERSION = 'INVALID'

    private Project project

    private AsakusaLangBaseExtension extension
     ...
     */
    static AsakusaLangBaseExtension get(Project project) {
        project.apply plugin: AsakusaLangBasePlugin
        return project.plugins.getPlugin(AsakusaLangBasePlugin).extension
    }
    ...
    void apply(Project project) {
        ...
        this.extension = project.extensions.create('asakusaLangBase', AsakusaLangBaseExtension)
        configureExtension()
        configureTasks()
    }
    ...
    private void extendVersionsTask() {
        project.tasks.getByName(AsakusafwBasePlugin.TASK_VERSIONS).doLast {
            logger.lifecycle "Asakusa Language: ${extension.featureVersion}"
        }
    }
}
