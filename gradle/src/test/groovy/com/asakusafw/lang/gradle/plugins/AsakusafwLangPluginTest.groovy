 ...
 */
package com.asakusafw.lang.gradle.plugins
...
/**
 * Test for {@link AsakusafwLangPlugin}.
 */
class AsakusafwLangPluginTest {
    ...
    public final TestRule initializer = new TestRule() {
        Statement apply(Statement stmt, Description desc) {
            ...
            project.apply plugin: 'asakusafw-lang'
            return stmt
        }
    }
    ...
    public void activate_asakusafw_sdk() {
        assert project.plugins.hasPlugin(AsakusaLangSdkPlugin) == false
        assert project.plugins.hasPlugin(AsakusaLangOrganizerPlugin) == false

        project.apply plugin: 'asakusafw-sdk'
        assert project.plugins.hasPlugin(AsakusaLangSdkPlugin)
        assert project.plugins.hasPlugin(AsakusaLangOrganizerPlugin) == false
    }
    ...
    public void activate_asakusafw_organizer() {
        ...
        assert project.plugins.hasPlugin(AsakusaLangSdkPlugin) == false
        assert project.plugins.hasPlugin(AsakusaLangOrganizerPlugin)
    }
}
